  ...
  export default {
    ...
    created() {
      ...
      if (userId) {
        ...
        Promise.all([
          ...
          this.form = res1;
          var roleIds = [];
          for (var index in res1.roles) {
            roleIds.push(res1.roles[index].id)
          }

          this.roles = res2.rows;
          this.total = res2.rows.length;

          this.$nextTick(() => {
            this.roles.forEach((row) => {
              // if (roleIds.includes(row.id) && row.flag) {
    ...
    methods: {
      ...
      handleSelectionChange(selection) {
        this.roleIds = selection.map((item) => parseInt(item.id));
      },
      // 保存选中的数据编号
      getRowKey(row) {
